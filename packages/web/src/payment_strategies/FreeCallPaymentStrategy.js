...
class FreeCallPaymentStrategy {
  ...
  async _generateSignature(currentBlockNumber) {
    ...
    return this._serviceClient.signData(
      ...
      {
        t: "bytes",
        v: tokenToMakeFreeCall.substring(2, tokenToMakeFreeCall.length),
      }
    );
  }
  ...
  async _getFreeCallStateRequest() {
    const Request =
      this._freeCallStateServiceClient.getFreeCallsAvailable.requestType;
